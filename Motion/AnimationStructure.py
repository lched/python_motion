...
def children_list(parents):
    ...
    def joint_children(i):
        return [
            j for j, p in enumerate(parents) if not isinstance(p, tuple) and p == i
        ]  # todo: 'isinstance' is a hack. change later
...
def get_sorted_order_internal(sorted_order, parent_out_idx, parent_in_idx, children):
    ...
    for child in children[parent_in_idx]:
        ...
        sorted_order, out_idx = get_sorted_order_internal(
            sorted_order, out_idx, child, children
        )
...
def get_sorted_order(parents):
    parents = np.array(
        parents
    )  # if type(parents)==list, make it an np.array. if it's already an np.array, this line does not change it.
    children = children_list(parents)
