...
class Quaternions:
    ...
    def interpolate(self, ws):
        return Quaternions.exp(
            np.average(abs(self).log(), axis=0, weights=ws)[np.newaxis]
        )
    ...
    def id(cls, n):
        ...
        if (
            isinstance(n, int) or isinstance(n, np.intc) or isinstance(n, np.int_)
        ):  # 'long' is not supported on python3. np.intc is equivalent to int, np.int_ is equivalent to long
            qs = np.zeros((n, 4))
            qs[:, 0] = 1.0
            return Quaternions(qs)
    ...
    def exp(cls, ws):
        ts = (
            np.sum(ws**2.0, axis=-1) ** 0.5
        )  # compute the norm for each entry. equivalent to np.linalg.norm(ws, axis=-1)
        ts[ts == 0] = 0.001
        ls = np.sin(ts) / ts
