...
def eye(shape, dtype=np.float32):
    return np.ones(list(shape) + [4], dtype=dtype) * np.asarray(
        [1, 0, 0, 0], dtype=dtype
    )
...
def log(x, eps=1e-5):
    ...
    halfangle = np.where(
        length < eps, np.ones_like(length), np.arctan2(length, x[..., 0:1]) / length
    )
...
def fk_vel(lrot, lpos, lvel, lang, parents):
    ...
    for i in range(1, len(parents)):
        ...
        gv.append(
            ...
            + _fast_cross(
                ga[parents[i]], mul_vec(gr[parents[i]], lpos[..., i : i + 1, :])
            )
            + gv[parents[i]]
        )
